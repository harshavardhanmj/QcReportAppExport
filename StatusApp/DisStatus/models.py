...
def pre_save_statusChangeQc_receiver(sender, instance, *args, **kwargs):
	if instance.CurrentStatus == "Testing - Completed":
		qs = ProjectQC.objects.get(ProductName__iexact=instance.ProductName)
		if qs:
			qs.ProductStatus = PROJ_STATUS[1][1]
			qs.save()
pre_save.connect(pre_save_statusChangeQc_receiver, sender=DailyStatusQC)
...
def pre_save_statusChangePq_receiver(sender, instance, *args, **kwargs):
	if instance.CurrentStatus == "PQ completed":
		qs = ProjectPq.objects.get(ProductName__iexact=instance.ProductName)
		if qs:
			qs.ProductStatus = PROJ_STATUS[1][1]
			qs.save()
pre_save.connect(pre_save_statusChangePq_receiver, sender=DailyStatusPQ)
...
def pre_save_creDateAuto_receiver(sender, instance, *args, **kwargs):
	if instance.PlanStartDate:
		if not instance.TempPlanStartDate:
			#createObj = ProjectQC.objects.get(Q(ProductName_iexact=instance.ProductName) & Q(ReleaseVersion_iexact=instance.ReleaseVersion))
